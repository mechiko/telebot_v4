...
func NewRecoveryInterface(l *zap.SugaredLogger) entity.RecoverInterface {
...
func (ri *recoverInterface) RecoverLog(str string) {
	if r := recover(); r != nil {
		...
		ri.logger.Errorf("RecoverLog %s", err.Error())
...
func (ri *recoverInterface) RecoverLogWithStack(str string) {
	if r := recover(); r != nil {
		...
		stack = stack[:runtime.Stack(stack, false)]
		err := fmt.Errorf("%v %v %v", str, time.Now(), r)
		ri.logger.Errorf("RecoverLogWithStack %s", err.Error())
		ri.logger.Errorf("Stack %s", string(stack))
...
func (ri *recoverInterface) RecoverFunc(str string) {
	...
	if r := recover(); r != nil {
		...
		ri.logger.Errorf("RecoverFunc %s", err.Error())
...
func (ri *recoverInterface) RecoverExit(str string) {
	if r := recover(); r != nil {
		err := fmt.Errorf("%v %v %v", str, time.Now(), r)
		_ = os.WriteFile("error.txt", []byte(err.Error()), 0644)
		ri.logger.Errorf("RecoverExit %s", err.Error())
